...
summary(rr_anova)
# rr_anova_table = apa.aov.table(rr_anova) # can't figure out how to edit AND export
# rr_anova_table$table_body$Predictor = c("Intercept",
#                                         "Conv. Type",
#                                         "Condition",
#                                         "Type x Condition",
#                                         "Error")
apa.aov.table(rr_anova,
              file = "./tables/rr_anova_table-vic.doc")

# effects on determinism
det_anova = aov(DET ~ conv_type * condition, data = analysis_crqa_df)
summary(det_anova)
apa.aov.table(det_anova,
              file = "./tables/det_anova_table-vic.doc")

# effects on maximum line
maxl_anova = aov(maxL ~ conv_type * condition, data = analysis_crqa_df)
summary(maxl_anova)
apa.aov.table(maxl_anova,
              file = "./tables/maxl_anova_table-vic.doc")
...
tbl_regression(drp_lmer,
               label = list("ot1" ~ "Linear Lag",
                            "ot2" ~ "Quadratic Lag",
                            "conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/drp_lmer-vic.html")
...
summary(close_lmer)
tbl_regression(close_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/close_lmer-vic.html")
...
tbl_regression(undersood_partner_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/undersood_partner_lmer-vic.html")
...
tbl_regression(understood_me_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/understood_me_lmer-vic.html")
...
# interaction-level analyses: closeness
...
tbl_regression(exploratory_close_crqa_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_close_crqa_lmer-vic.html")

# interaction-level analyses: understood partner
...
tbl_regression(exploratory_understood_partner_crqa_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_understood_partner_crqa_lmer-vic.html")

# interaction-level analyses: understood me
exploratory_understood_me_crqa_lmer = lmer(mean_understood_me ~ (RR + DET + maxL) * 
                                             conv_type * condition + 
                                             (1 | dyad),
                                           data = analysis_exploratory_crqa_df)
summary(exploratory_understood_me_crqa_lmer)
tbl_regression(exploratory_understood_me_crqa_lmer,
               label = list("conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_understood_me_crqa_lmer-vic.html")
...
##### 3b. Moment-to-moment metrics (E1; 3.5) #####
...
tbl_regression(exploratory_close_drp_lmer,
               label = list("ot1" ~ "Linear Lag",
                            "ot2" ~ "Quadratic Lag",
                            "conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_close_drp_lmer-vic.html")

# dynamics analyses: understood partner
...
tbl_regression(exploratory_understood_partner_drp_lmer,
               label = list("ot1" ~ "Linear Lag",
                            "ot2" ~ "Quadratic Lag",
                            "conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_understood_partner_drp_lmer-vic.html")
...
tbl_regression(exploratory_understood_me_drp_lmer,
               label = list("ot1" ~ "Linear Lag",
                            "ot2" ~ "Quadratic Lag",
                            "conv_type" ~ "Conv. Type",
                            "condition" ~ "Condition")) %>%
  as_gt() %>%
  gt::gtsave(filename = "./tables/exploratory_understood_me_drp_lmer-vic.html")
...
plot_exploratory_drp_df = analysis_exploratory_drp_df %>%
  ...
  mutate(plot_closeness = factor(plot_closeness, 
                                 labels = c("Low Closeness", 
                                            "High Closeness")),
         plot_understood_partner = factor(plot_understood_partner, 
                                          labels = c("Low Understanding", 
                                                     "High Understanding")),
         plot_understood_me = factor(plot_understood_me, 
                                          labels = c("Low Feeling Understood", 
                                                     "High Feeling Understood")))
